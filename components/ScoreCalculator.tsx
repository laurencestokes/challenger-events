...
export default function ScoreCalculator({ activities, userProfileOverride }: ScoreCalculatorProps) {
  ...
  const getDefaultValue = (activity: Activity): number => {
    ...
    if (activity.scoringSystemId === 'bike_10km') {
      return 1080; // Default 10km bike time: 18 minutes
    }

    if (activity.scoringSystemId === 'ski_1km') {
      return 210; // Default 1km ski time: 3 minutes 30 seconds
    }
  ...
  const getMinMaxValues = (activity: Activity): { min: number; max: number } => {
    ...
    if (activity.scoringSystemId === 'bike_10km') {
      return { min: 600, max: 2400 }; // 10km bike: 10 minutes to 40 minutes
    }

    if (activity.scoringSystemId === 'ski_1km') {
      return { min: 120, max: 420 }; // 1km ski: 2 minutes to 7 minutes
    }
    ...
    try {
      ...
      switch (scoringSystem.calculationFunction) {
        ...
        case 'bike500mScore':
          ...
          break;
        case 'bike10kmScore':
          requiredRawInput = challengerData.getBike10kmTimeForScore(targetScore, sex, age);
          actualScore = challengerData.bike10kmScore(requiredRawInput, sex, age).score;
          break;
        case 'ski1kmScore':
          requiredRawInput = challengerData.getSki1kmTimeForScore(targetScore, sex, age);
          actualScore = challengerData.ski1kmScore(requiredRawInput, sex, age).score;
          break;
        case 'running1mileScore':
          requiredRawInput = challengerData.getRunning1mileTimeForScore(targetScore, sex, age);
