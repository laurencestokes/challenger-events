...
async function updateAdminUser(email, firebaseUid) {
  try {
...
// Usage: node scripts/update-admin-user.js <email> <firebase-uid>
const email = process.argv[2];
const firebaseUid = process.argv[3];

if (!email || !firebaseUid) {
  console.log('Usage: node scripts/update-admin-user.js <email> <firebase-uid>');
  console.log('');
  console.log('To find your Firebase UID:');
  console.log('1. Go to Firebase Console > Authentication > Users');
  console.log('2. Find your user and copy the UID');
  console.log('');
  console.log('Example: node scripts/update-admin-user.js admin@example.com abc123def456');
  process.exit(1);
}

updateAdminUser(email, firebaseUid);
